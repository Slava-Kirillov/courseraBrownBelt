...
public:
    ...
    struct WriteAccess {
        ...
        WriteAccess(const K &key, Bucket & bucket) : guard(bucket.m), ref_to_value(bucket.value[key]) {}
    };
    ...
    struct ReadAccess {
        ...
        ReadAccess(const K &key, const Bucket & bucket) : guard(bucket.m), ref_to_value(bucket.value.at(key)) {}
    };
    ...
    MapType BuildOrdinaryMap() const {
        ...
        for (auto& bucket: data) {
            auto g = Lock(bucket.m);
            result.insert(begin(bucket.value), end(bucket.value));
        }
        return result;
    }
