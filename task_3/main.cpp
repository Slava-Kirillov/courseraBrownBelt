...
struct AddressHasher {
    ...
    size_t operator()(const Address &address) const {
        const hash<string> hstring;
        const hash<int> hint;

        size_t r1 = hstring(address.city);
        size_t r2 = hstring(address.street);
        size_t r3 = hint(address.building);
        const size_t coef = 2'946'901;

        return coef * coef * r1 + coef * r2 + r3;
    }
};
...
struct PersonHasher {
    size_t operator()(const Person &person) const {
        ...
        const hash<string> hstring;
        const hash<int> hint;
        const hash<double> hdouble;
        const AddressHasher haddress;

        size_t r1 = hstring(person.name);
        size_t r2 = hint(person.height);
        size_t r3 = hdouble(person.weight);
        size_t r4 = haddress(person.address);

        return coef * coef * coef * r1 +
               coef * coef * r2 +
               coef * r3 + r4;
    }
